...
def publication_checks(issues):
 ...
 paths=[ROOT/'FundationsDataScience.pdf',*[ROOT/'chapters-pdf'/f'{name}.pdf' for name in chapter_list()],ROOT/'figure-processing/figure-comparisons.pdf']
 results=[]
 for path in paths:
 ...
 for name in chapter_list():
  full=labels(ROOT/'build/book/chapters'/f'{name}.aux');single=labels(ROOT/'build/chapters'/name/f'{name}.aux')
...
def main():
 items=json.loads((ROOT/'figure-processing/figure-project.json').read_text());by_id={i['id']:i for i in items};issues=[];report={};originals=set();props=set();expected=set();accepted=set();pending=set()
 ...
 index_path=ROOT/'figure-processing/figure-index.json'
 if index_path.exists():
  idx=json.loads(index_path.read_text())
  review_items=[i for i in items if i.get('comparison_visible',i['engine']!='tikz')]
  if len(idx)==len(review_items):
   comp=PdfReader(ROOT/'figure-processing/figure-comparisons.pdf');book=PdfReader(current);dest=book.named_destinations;badlinks=[];links=0
