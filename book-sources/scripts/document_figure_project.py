...
def main():
    ...
    index = {i['id']: i for i in json.loads((ROOT / 'figure-processing/figure-index.json').read_text())}
    ...
    for directory, figures in groups.items():
        ...
        for item in figures:
            ...
            if state not in ('keep-original', 'removed', 'merged'):
                ...
                text += 'build/figure-runtime/bin/python scripts/regenerate_figures.py --id ' + item['id'] + '\n```\n\n'
    ...
    (ROOT / 'figure-processing/figure-index.md').write_text(catalog)
    ...
    (ROOT / 'figure-processing/author-decisions.json').write_text(json.dumps(decisions, indent=2)+'\n')
