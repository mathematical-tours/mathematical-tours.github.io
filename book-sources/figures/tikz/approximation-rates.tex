...
\foreach \a/\title in {0/{Sobolev 1-D},2.45/{Piecewise smooth},4.9/{Sobolev 2-D},7.35/{Bounded variation},9.8/{Cartoon, $C^2$}}{
...
\end{scope}
\node[anchor=north,inner sep=0] (rate-table) at (5.95,.08) {\renewcommand{\arraystretch}{1.45}\begin{tabular}{lccccc}
\hline
$\|f-f_M\|_2^2\lesssim$ & Sobolev 1-D & piecewise 1-D & Sobolev 2-D & BV 2-D & cartoon 2-D\\\hline
...
\end{tabular}};
\node[anchor=north,align=center,text width=12cm] at ([yshift=-6pt]rate-table.south) {Cartoon images: adaptive triangulations $O(M^{-2})$; curvelets $O(M^{-2}(\log M)^3)$.\\L: linear; NL: nonlinear. Sobolev columns: \mbox{$H^\alpha$, $\alpha\geq1$}.\\Use compatible wavelet bases and boundary conditions.\\The BV class also has the uniform bound assumed in the chapter.};
\end{tikzpicture}
\end{document}
