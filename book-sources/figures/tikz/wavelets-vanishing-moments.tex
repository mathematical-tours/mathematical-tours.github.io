...
\node at (3,-1.6) {$\widehat h^{(k)}(\pi)=\widehat g^{(k)}(0)=0,\qquad k=0,\ldots,p-1$};
\node at (3,-2.35) {$\Longleftrightarrow\qquad\displaystyle\int_{\mathbb R}x^k\psi(x)\,\mathrm{d}x=0,\qquad k=0,\ldots,p-1$};
\node[muted,font=\small] at (3,-3.15) {Curves show magnitudes; derivatives refer to the filter symbols.};
