...
`scripts/regenerate_figures.py` rebuilds accepted and pending reconstructions, skipping explicitly retained originals and removed/merged entries. `scripts/build_book.py` generates figures before compiling the book and independent chapters. `scripts/build_figure_comparator.py` refreshes current numbers and captions and handles an empty pending list with a completion page. `scripts/document_figure_project.py` updates source guides, index and decision ledger. `scripts/audit_figure_project.py` verifies exact reading inputs, historical and data hashes, publication fonts, labels, dates and links.
