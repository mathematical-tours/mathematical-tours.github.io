...
Run `make` from the repository root to regenerate changed figures and rebuild the book, chapter PDFs and review status. Run `make figure-audit` to check the exact accepted reading inputs, original/data hashes, embedded fonts, labels, dates and PDF destinations. The latest report is `verification.json`.
