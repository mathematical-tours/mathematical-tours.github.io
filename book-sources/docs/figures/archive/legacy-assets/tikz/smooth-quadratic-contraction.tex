...
\node[anchor=west,font=\small] at (axis cs:.67,.70) {$\rho^\star=\dfrac{L-\mu}{L+\mu}$};
\end{axis}
\node[anchor=north,font=\footnotesize] at (current bounding box.south)
  {$\mu=\lambda_{\min}(C)>0,\quad L=\lambda_{\max}(C)\qquad\text{(drawn for }L=3\mu\text{)}$};
\end{tikzpicture}
\end{document}
