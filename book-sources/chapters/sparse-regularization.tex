...
\eq{
	f_{\la,q}^{\star} = \sum_m (a_{\la,q}^{\star})_m \psi_m
}
\eq{
	a_{\la,q}^{\star} = \uargmin{b \in \RR^N} \sum_m \frac{1}{2} |a_m-b_m|^2 + \la \phi_q(b_m)
}
where $a_m=\dotp{f}{\psi_m}$, and with 
\eq{
	\phi_1(x)=|x|
	\qandq
	\phi_0(x)=
...
\eq{
	(a_{\la,q}^{\star})_m = \uargmin{\al \in \RR}  \frac{1}{2} |a_m-\be|^2 + \la \phi_q(\be)
}
...
\eql{\label{eq-soft-thresh}
	S_T^1(r) \eqdef  \sign(r) ( |r|-\la )_+
} 
is the soft thresholding introduced in~\eqref{eq-soft-thresh-denoise}.
\end{prop}
\begin{proof}
	TODO. Put a plot of the thresholding functions.
\end{proof}
...
\eq{
	\Psi : a \in \RR^Q \mapsto \Psi a = \sum_m a_m \psi_m
	\qandq
...
\eql{\label{eq-sparsity-synthesis}
	J_1^{\text{S}}(f) \eqdef \umin{a \in \RR^q, \Psi a = f} \norm{a}_1.  
}
...
For the special case~\eqref{eq-bpdn}, one can consider a splitting of the form~\eqref{eq-fb-split} defining
...
%
In the case~\eqref{eq-bpdn-func}, Proposition~\eqref{prop-equiv-sparse-thresh} shows that $\Prox_{\rho J}$ is the soft thresholding, so that~\eqref{eq-fb} now reads 
\eql{\label{eq-ista}
