...
Left: linear regression. Middle: linear classifier. Right: classification loss.
}
\end{figure}
...
Left: no minimizer. Middle: multiple minimizers. Right: a unique minimizer.
}
\end{figure}
...
Convex and nonconvex functions; strictly convex and convex but not strictly convex functions.
}
\end{figure}
...
When $f$ is convex, one can strengthen the condition~\eqref{eq-convexity-def} and impose that the inequality is strict for distinct $x,y$ and $t\in]0,1[$ (see Fig.~\ref{fig-cvx-vs-noncvx}, right), i.e.
...
Local extrema (left), a stationary inflection point (middle), and a global minimizer (right).
}
\end{figure}
...
Left: point clouds $(a_i)_i$ and their principal directions. Right: the quadratic part of $f(x)$.
}
\end{figure}
...
Since $L(z)=\sum_i\ell(z_i)$, its gradient is $\nabla L(z)=(\ell'(z_i))_{i=1}^n$. For logistic loss, $\ell'(u)=e^u/(1+e^u)$. Evaluated at the negative margin $u=-y_i\dotp{x}{a_i}$, this sigmoid is large for incorrectly classified observations and weights their contribution to the gradient.
...
\caption{\label{fig-expansion-taylor}
	Left: first-order Taylor approximations in one and two dimensions.
	Right: the gradient is orthogonal to the level set; the diagram illustrates the proof.
}
\end{figure}
...
\begin{rem}[Orthogonality to level sets]
	...
	Assume $f$ is continuously differentiable near $x$ and $\nabla f(x)\neq0$. Then $x$ is a regular point of the level set $\Ll_{f(x)}$. The gradient is normal to its tangent space and points toward increasing values of $f$; see Figure~\ref{fig-expansion-taylor}, right.
	%
...
Effect of the step size $\tau$ on gradient descent (left) and exact line search (right).
}
\end{figure}
...
Contraction constant $h(\tau)$ for a quadratic function.
}
\end{figure}
