...
\eq{
	\partial f(x) = A_{t} \times A_{t-2} \times \ldots A_2 \times A_1.
}
...
Complexity of forward (left) and backward (right) modes for feedforward graphs.  \todo{redo the figure}
}
\end{figure}
...
\eq{
	...
	\qandq
	s_{k+1} = s_k + \tau F(r_{k+1},\th_{k+1/2},\tau (k+1/2)).
}
One can reverse these equation exactly as
\eq{
	s_k = s_{k+1} - \tau F(r_{k+1},\th_{k+1/2},\tau (k+1/2)).
	\qandq
...
\eq{
	...
	[\partial_\th h(x,\th)]^\top = \diag\pa{\frac{1}{\Kk x}}, \quad
}
\eq{
	\nabla_x L(x,\th) = ??, \quad
	\nabla_\th L(x,\th) = ??. 
...
Computational graph for a MLP. \todo{redo the figure}
}
\end{figure}
