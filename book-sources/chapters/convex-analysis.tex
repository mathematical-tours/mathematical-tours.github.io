...
One checks that $\partial f(x) \subset \Hh^*$ is a closed convex set.
...
\eq{
	\partial \iota_\Cc(x) = \Nn_\Cc(x) \eqdef \enscond{ y-x }{ \foralls z \in \Cc, \dotp{z-x}{y-x} \leq 0 }.
}
For an affine space $\Cc = a+\Vv$ where $\Vv \subset \Hh$ is a linear space, then $\Nn_\Cc(x)=\Vv^\bot$ is the usual orthogonal for linear spaces. If $x \in \int(\Cc)$ is in the interior of $\Cc$, then $\Nn_\Cc(x)=\RR^N$.  
...
\eq{
	p^\star = \umin{x} \umax{u \in \RR^P, v \in \RR_-^Q } \Ll(x,u,v) \eqdef f(x) + \dotp{Ax-y}{u} + \dotp{g(x)}{v} .
}

It is tempting to inverse the inf and the sup, and study 
\begin{align}\label{eq-lagrange-dual}
	d^\star = \umax{(u,v) \in \RR^P \times \RR_+^Q } F(u,v) &\eqdef \umin{x}  f(x) + \dotp{Ax-y}{u} + \dotp{g(x)}{v}	
\end{align}
One remark that $F$ is a concave function (as being the minimum of linear forms), and this ``dual'' problem is thus a maximization of a concave function. 
...
\begin{prop}
	One always have, for all $(u,v) \in \RR^P \times \RR_+^Q$, % for $x$ such that $Ax=y, g(x) \leq 0$ and 
	\eq{
...
The following fundamental theorem, more difficult to prove give a sufficient condition (so-called qualification of the constraints) such that one actually has equality.
...
\eq{
	(f \otimes g)(x) \eqdef \sup{ y+y'=x } f'(y) + g(y').
}
...
\begin{align}\label{eq-deriv-rock-fench}
	d &= \usup{u} \umin{(x,y)} f(x) + g(y) + \dotp{Ax-y}{u} \\
