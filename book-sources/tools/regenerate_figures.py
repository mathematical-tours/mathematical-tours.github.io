...
PROJECT=ROOT/'docs/figures/figure-project.json'
...
def compile_tex(source,output,job):
    ...
    dependencies=[source,*sorted((ROOT/'chapters-tex/shared').glob('*.tex')),
                  *sorted((ROOT/'chapters-tex/shared').glob('*.sty')),ROOT/'figures-code/tikz-preamble.tex']
...
def main():
    ...
    from build_book import ensure_book_references
    ensure_book_references()
