...
def build_comparisons(preview=False,chapter_filter=None):
    ...
    for item in items:
        ...
        if item['book_label']:
            ...
            aux=(ROOT/'build/book/chapters-tex'/f"{item['chapter']}.aux").read_text()
    ...
    (ROOT/'docs/figures/figure-project.json').write_text(json.dumps(items,indent=2,ensure_ascii=False)+'\n')
    ...
    external=r'\HyperFirstAtBeginDocument{}'+'\n'+(ROOT/'build/figure-review/book-external.aux').read_text();pages=[];index=[];chapter=None
    for item in ready:
    ...
    if not items:
        ...
        accepted=sum(i.get('review_state')=='accepted' for i in json.loads((ROOT/'docs/figures/figure-project.json').read_text()))
    ...
    target=BUILD/'preview-comparisons.pdf' if preview else ROOT/'docs/figures/figure-comparisons.pdf'
    shutil.copy2(report['pdf'],target)
    (BUILD/'complete-figure-index.json').write_text(json.dumps(index,indent=2,ensure_ascii=False)+'\n')
    if not preview:shutil.copy2(BUILD/'complete-figure-index.json',ROOT/'docs/figures/figure-index.json')
