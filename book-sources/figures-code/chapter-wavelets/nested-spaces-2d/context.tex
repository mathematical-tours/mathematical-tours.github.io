...
anisotropic 2-D transform follows the same separable construction as the 2-D FFT in Section~\ref{sec-fft-2d}. Treat the image $a_J \in \RR^{2^{-J} \times 2^{-J}}$ as a matrix, apply the 1-D FWT to each row, and then apply it to each column. The total cost is $O(N)$ for $N=2^{-2J}$ pixels.
...
\eq{
	\foralls n\in \{0,\ldots,N-1\}^2, \quad a_{J,n}