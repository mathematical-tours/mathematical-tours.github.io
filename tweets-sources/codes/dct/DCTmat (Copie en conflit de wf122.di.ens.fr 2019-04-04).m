...
A1([1,end],:) = A1([1,end],:)/sqrt(2);
imwrite(rescale(Upsc(A1,8)), [rep 'dct1.png']);

A2 = cos((X+1/2).*Y*pi/n);
A2(:,1) = A2(:,1)/sqrt(2);
imwrite(rescale(Upsc(A2,8)), [rep 'dct2.png']);


A4 = cos((X+1/2).*(Y+1/2)*pi/n)*sqrt(2/n);
imwrite(rescale(Upsc(A4,8)), [rep 'dct4.png']);


Cas= ( cos(X.*Y*2*pi/n) + sin(X.*Y*2*pi/n) ) * 1/sqrt(n);
imwrite(rescale(Upsc(Cas,8)), [rep 'hartley.png']);
